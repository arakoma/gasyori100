...
def emboss_filter(img_, k_size=3):
    if len(img_.shape) == 2:
        img = img_.copy()
    ...
    #kernel
    kernel = np.array([[-2, -1, 0], [-1, 1, 1], [0, 1, 2]]).astype(np.float32)
...
out = emboss_filter(img_gray).astype(np.uint8)

cv2.imwrite(r"out/out_18.jpg", out)
cv2.imshow("", out)
cv2.waitKey(0)

...
def prewitt_filter(img_, k_size=3):
    ...
    kernel_v = np.array([[-1, -1, -1], [0, 0, 0], [1, 1, 1]]).astype(np.float32)
...
out_v, out_h = prewitt_filter(img_gray)
out_v = out_v.astype(np.uint8)
out_h = out_h.astype(np.uint8)

cv2.imwrite(r"out/out_16_v.jpg", out_v)
cv2.imshow("", out_v)
cv2.waitKey(0)
cv2.destroyAllWindows

cv2.imwrite(r"out/out_16_h.jpg", out_h)
cv2.imshow("", out_h)
cv2.waitKey(0)
cv2.destroyAllWindows